...
# START - путь для новой даты

# Путь откуда мы ходим извлеч новые данные
...
def update_date_of_sale(date_input):
    """
    Обновляем формат from DD/MM/YYYY to YYYY-MM-DD
    """
...
def update_description(description_input):
    """
    Обновляем описание для облегчение анализа:
...
def update_price(price_input):
    """
    Очищаем данные с ценами удаляя запитые и симфолы:
    - "€" symbol
...
def transform_new_data():
    ...
    with open(raw_path, mode="r", encoding="windows-1252") as csv_file:
        ...
        # создаем пустой список PprRawAll objects
        ppr_raw_objects = []
        for row in reader:
            # делаем трасформанию и сохраняем как PprRawAll object
